...
class Intercom_DWT(Intercom_empty):
        ...
        def send_bitplane(self, arr_data, bitplane_number):
            ...
            #sys.stderr.write("\nARR {}".format(arr_data[:, bitplane_number%self.number_of_channels])); sys.stderr.flush()
            if np.any(bitplane):
                ...
                self.skipped_bitplanes[(self.recorded_chunk_number+1) % self.cells_in_buffer] += 1
        ...
        def send(self, indata):
            ...
            sys.stderr.write("\nNOBPTS {} LASTBPTS {}".format(self.NOBPTS, last_BPTS)); sys.stderr.flush()
